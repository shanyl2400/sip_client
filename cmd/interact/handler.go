...
func (p *Prompt) parseCreateSession(args []string) (*sip.CreateSessionRequest, error) {
	params := p.parseArgs(args)
	deviceID := params["d"]
	channelID := params["c"]
	ip := params["i"]
	...
	return &sip.CreateSessionRequest{
		DeviceID:  deviceID,
		ChannelID: channelID,
		IP:        ip,
		UdpPort:   port,
...
func (p *Prompt) parseReleaseSession(args []string) (*sip.ReleaseSessionRequest, error) {
	params := p.parseArgs(args)
	deviceID := params["d"]
	dialogID := params["di"]
	channelID := params["c"]
	...
	return &sip.ReleaseSessionRequest{
		DeviceID:  deviceID,
		DialogID:  dialogID,
		ChannelID: channelID,
		Ssrc:      uint32(ssrc),
...
func (p *Prompt) parseRecoverSession(args []string) (*sip.RecoverSessionRequest, error) {
	params := p.parseArgs(args)
	deviceID := params["d"]
	dialogID := params["di"]
	channelID := params["c"]
	...
	return &sip.RecoverSessionRequest{
		DeviceID:  deviceID,
		DialogID:  dialogID,
		ChannelID: channelID,
		Ssrc:      uint32(ssrc),
...
func (p *Prompt) parseCameraCtrl(args []string) (*sip.CameraCtrlRequest, error) {
	params := p.parseArgs(args)
	cameraId := params["c"]
	actionStr := params["a"]
	paramStr := params["p"]
...
func (p *Prompt) parseAddDevice(args []string) *sip.AddDeviceRequest {
	params := p.parseArgs(args)
	deviceID := params["d"]
	password := params["p"]
	scope := params["s"]

	return &sip.AddDeviceRequest{
		DeviceID: deviceID,
		Password: password,
		Scope:    scope,
	}
